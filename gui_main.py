...
from .gui_live import WindowLv2_TrainTransformer, WindowLv2_LivePredictionSettings, WindowLv2_LiveAnalysisSettings
...
class InitialWindow(wx.Frame):
	...
	def display_window(self):
		...
		# Horizontal spacer before Prediction Module
		boxsizer.Add(0, 10, 0)

		# Prediction Module (Centered at Bottom)
		prediction_sizer = wx.BoxSizer(wx.HORIZONTAL)
		
		button_predict = wx.Button(panel, label='Live Module', size=(200, 40))
		button_predict.Bind(wx.EVT_BUTTON, self.window_live)
		wx.Button.SetToolTip(button_predict, 'Perform live analysis or train a transformer model to predict future behavior sequences for application in live videos.')

		prediction_sizer.Add(button_predict, 0, wx.ALIGN_CENTER)  # Centered button
		boxsizer.Add(prediction_sizer, 0, wx.ALIGN_CENTER, 50)
	...
	def window_live(self,event):

		WindowLv1_LiveModule('Live Module')
...
class WindowLv1_LiveModule(wx.Frame):
	...
	def __init__(self,title):
		...
		super(WindowLv1_LiveModule,self).__init__(parent=None,title=title,size=(500,350))
		self.display_window()
	...
	def display_window(self):
		...
		button_live_analysis=wx.Button(panel,label='Live Analysis',size=(300,40))
		button_live_analysis.Bind(wx.EVT_BUTTON,self.live_analysis)
		wx.Button.SetToolTip(button_live_analysis,'Perform live analysis on a live subject.')
		boxsizer.Add(button_live_analysis,0,wx.ALIGN_CENTER,10)
		boxsizer.Add(0,20,0)

		button_train_transformer=wx.Button(panel,label='Train Transformer',size=(300,40))
		button_train_transformer.Bind(wx.EVT_BUTTON,self.train_transformer)
		wx.Button.SetToolTip(button_train_transformer,'Train a transformer model on LabGym CV output data.')
		boxsizer.Add(button_train_transformer,0,wx.ALIGN_CENTER,10)
		boxsizer.Add(0,20,0)

		button_live_prediction=wx.Button(panel,label='Start Live Analysis and Predictions',size=(300,40))
		button_live_prediction.Bind(wx.EVT_BUTTON,self.live_prediction)
		wx.Button.SetToolTip(button_live_prediction,'Employ a trained transformer model with live analysis.')
		boxsizer.Add(button_live_prediction,0,wx.ALIGN_CENTER,10)
		boxsizer.Add(0,20,0)
	...
	def train_transformer(self,event):

		WindowLv2_TrainTransformer('Train Transformer')

	def live_prediction(self,event):

		WindowLv2_LivePredictionSettings('Start Prediction')

	def live_analysis(self,event):

		WindowLv2_LiveAnalysisSettings('Live Analysis')
